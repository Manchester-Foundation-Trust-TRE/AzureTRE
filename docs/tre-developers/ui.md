...
- Next.js v16 with React v18
  - Typescript
  - Next.js App Router for routing
  - React Router v6 for client-side routing compatibility
...
│   │   ├── providers.tsx  - Client-side providers (MSAL, Redux, Router)
│   │   └── logout         - Logout page route
│   ├── out                - Location of compiled static files after build process
│   ├── public             - Location for static assets (favicon, manifest, etc.)
...
- Build the source code via `yarn build`, which runs Next.js build with static export
- Deploy the compiled static files from the `out/` directory to Azure blob storage, where it will be statically served behind the App Gateway that also fronts the API.
...
In the UI directory (`ui/app/`), you can run:

### `npm run dev` or `yarn dev`

Runs the app in development mode with Next.js.<br>
...
Builds the app for production to the `out/` folder with Next.js static export.<br>
...
Starts the Next.js production server (not used for static export deployment).<br>
...
Runs Next.js linter on the project.<br>
